...
/*
The idea behind this class is the following:
- We exposed the end-user to the 'requestPermissions()' method that accepts a variable number of arguments representing
permissions we are interested in.
- We can pass the callback (the interface we can defined) that is optional.
- If we do not pass our own callback, the default one will be used.
- After permissions are resolved, we fire callbacks.
- We consider permissions resolving successful, only if permissions are granted.
